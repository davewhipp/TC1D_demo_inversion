...
import sys  # BG: Required for MPI shutdown
...
def prep_model(params):
    ...
    if params["cmd_line_call"]:
        ...
        if not params["batch_mode"]:
            ...
            # BG: Choose between NA and MCMC explicitly
            if params.get("inverse_mode", "NA").upper() == "MCMC":
                batch_run_mcmc(params, batch_params)
            elif params.get("inverse_mode", "NA").upper() == "NA":
                batch_run_na(params, batch_params)
            else:
                raise ValueError(f"Unknown inversion mode: {params.get('inverse_mode')}")
...
# ========== MCMC helper functions (for multiprocessing and MPI) ========== BG
...
def log_prior(x):
    ...
    if "ero_option3" in param_dict and param_dict["ero_option3"] > global_max_exhumation - ero1:
        return -np.inf
    if "ero_option5" in param_dict:
        ero3 = param_dict.get("ero_option3", 0.0)
        if param_dict["ero_option5"] > global_max_exhumation - (ero1 + ero3):
            return -np.inf
    if "ero_option7" in param_dict:
        ero3 = param_dict.get("ero_option3", 0.0)
        ero5 = param_dict.get("ero_option5", 0.0)
        if param_dict["ero_option7"] > global_max_exhumation - (ero1 + ero3 + ero5):
            return -np.inf  # BG: Ensure total thickness including 3rd interval <= max
    if "ero_option9" in param_dict:
        ero3 = param_dict.get("ero_option3", 0.0)
        ero5 = param_dict.get("ero_option5", 0.0)
        ero7 = param_dict.get("ero_option7", 0.0)
        if param_dict["ero_option9"] > global_max_exhumation - (ero1 + ero3 + ero5 + ero7):
            return -np.inf  # BG: Ensure total thickness including 4th interval <= max
    return 0.0
...
# ========== Batch MCMC Runner ========== BG

def batch_run_mcmc(params, batch_params):
    """Runs TC1D in batch mode with MCMC inversion"""

    param_list = list(ParameterGrid(batch_params))
    print(f"--- Starting batch processor for {len(param_list)} models ---\n")

    if params.get("inverse_mode", "NA").upper() != "MCMC":  # BG: Only run this block if MCMC is selected
        print("[INFO] Skipping MCMC block since inverse_mode is not 'MCMC'")
        return
    ...
    # BG: Extract parameters that are being varied to define bounds of the inversion
    ...
    # BG: Initialize pool if possible
    use_mpi = False
    try:
    ...
    if use_mpi:
        ...
        if not pool.is_master():
            pool.wait()
            sys.exit(0)
        print("- Running MCMC with MPI Pool")
    else:
        pool = None
        print("- Running MCMC in serial mode")

    # BG: Initialize sampler
    sampler = emcee.EnsembleSampler(
        nwalkers,
        ndim,
        log_probability,
        pool=pool  # BG: Use MPI pool here if applicable
    )
    ...
    print("[MCMC] Chain plot saved as:", os.path.abspath("mcmc_chains.png"))
    ...
    for i, j in combinations(range(ndim), 2):
        ...
        plt.savefig(f"mcmc_scatter_{param_names[i]}_vs_{param_names[j]}.png")
        plt.close(fig)
    ...
    print("[MCMC] Chain plot saved as:", os.path.abspath("mcmc_chains.png"))
    print("[MCMC] Corner plot saved as:", os.path.abspath("mcmc_corner.png"))
    if ndim >= 2:
        print("[MCMC] Pairwise scatter plots saved (one per parameter pair).")
...
def batch_run_na(params, batch_params):
    ...
    if params.get("inverse_mode", "NA").upper() == "NA":  # BG: Only run NA block if inverse_mode is 'NA'
        ...
        def objective(x):
            ...
            if "ero_option3" in filtered_params:
                ero3_final = max(0, min(ero3_final, max_exhumation - ero1))
            ...
            if "ero_option5" in filtered_params:
                ero5_final = max(0, min(ero5_final, max_exhumation - (ero1 + ero3_final)))
                filtered_params["ero_option5"] = ero5_final

            # BG: Apply constraint to ero_option7 if part of inverted parameters
            if "ero_option7" in filtered_params:
                ero7_final = max(0, min(ero7_final, max_exhumation - (ero1 + ero3_final + ero5_final)))
                filtered_params["ero_option7"] = ero7_final

            # BG: Apply constraint to ero_option9 if part of inverted parameters
            if "ero_option9" in filtered_params:
                ero9_final = max(0, min(ero9_final, max_exhumation - (ero1 + ero3_final + ero5_final + ero7_final)))
        ...
        for i in searcher.samples:
            ...
            if "ero_option3" in param_dict:
                param_dict["ero_option3"] = min(max_exhumation - ero1, ero3)
            if "ero_option5" in param_dict:
                param_dict["ero_option5"] = min(max_exhumation - (ero1 + param_dict.get("ero_option3", 0.0)),
                                                param_dict["ero_option5"])
            if "ero_option7" in param_dict:
                param_dict["ero_option7"] = min(
                    max_exhumation - (ero1 + param_dict.get("ero_option3", 0.0) + param_dict.get("ero_option5", 0.0)),
                    param_dict["ero_option7"]
                )  # BG: Constrain 3rd interval thickness
            if "ero_option9" in param_dict:
                param_dict["ero_option9"] = min(
                    max_exhumation - (ero1 + param_dict.get("ero_option3", 0.0) + param_dict.get("ero_option5", 0.0) + param_dict.get("ero_option7", 0.0)),
                    param_dict["ero_option9"]
                )  # BG: Constrain 4th interval thickness
            # Re-convert to list
        ...
        if "ero_option3" in best_dict:
            best_dict["ero_option3"] = min(max_exhumation - ero1, ero3)

        if "ero_option5" in best_dict:
            best_dict["ero_option5"] = min(max_exhumation - (ero1 + best_dict.get("ero_option3", 0.0)),
        ...
        plt.savefig("NA_covariance_matrix.png")
        if 'fig' in locals():
            plt.close(fig)
        ...
        print("[NA] Voronoi plot saved as:", os.path.abspath("voronoi.png"))
